...
/** Processes unprocessed webhook deliveries oldest-first, then prunes rows older than the 24h redelivery window (the natural poison-message bound for a delivery that keeps failing). */
export async function sweepWebhookDeliveries(deps: AutomationDeps): Promise<number> {
  const now = deps.clock.now();
  await deps.db
    .delete(webhookDeliveries)
    .where(lt(webhookDeliveries.receivedAt, new Date(now.getTime() - 24 * 60 * 60 * 1000)));
  ...
  for (;;) {
    const [delivery] = await deps.db
      ...
      .where(isNull(webhookDeliveries.processedAt))
    ...
    try {
      ...
      // Keep the row unprocessed — the next sweep retries; the 24h prune is
      // the bound. GitHub's own redelivery covers events lost to the prune.
      console.error(`automation delivery ${delivery.deliveryId} (${delivery.eventType}) failed`, error);
    }
    await deps.db
