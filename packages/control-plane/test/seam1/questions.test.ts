...
describe("Step human-in-the-loop (issue 13)", () => {
  ...
  it("edit-artifact is a group-authorized draft write: the uploaded bytes become an immutable human Artifact on the next StepRun", async () => {
    ...
    // Answering creates the next turn. Close that fixture so it cannot be
    // claimed by the following waiting-page tests in this shared rig.
    await rig.pool.query(
      "update step_runs set outcome = 'cancelled', reason = 'test-cleanup' where run_id = $1 and outcome = 'ready'",
      [runId],
    );
  });
